<script>
  ...
  import { JSR, ModuleRail, ModuleSlider, ModuleBar, ModuleLabel, ModuleGrid } from 'mm-jsr';
...
<style>
  ...
  .page-section--start_demo {
    min-width: 300px;
    width: 100%;
    max-width: 50vw;
  }

  .page-section--start_name {
    display: block;
    font-size: 0.75em
  }

  .page-section--start_app {
    margin-top: 0.5em;
    display: block
  }

  .page-section--start_subname {
    display: block;
    font-style: italic;
    font-size: 0.5em
  }
  ...
  .button {
    background: linear-gradient(30deg, #600, #d00);
